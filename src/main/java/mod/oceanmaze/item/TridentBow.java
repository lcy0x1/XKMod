...
public class TridentBow extends BowItem {

	public TridentBow(Settings settings) {
		super(settings);
	}

	@Override
	public Predicate<ItemStack> getProjectiles() {
		return (is) -> is.getItem() == Items.TRIDENT;
	}
	...
	public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
		...
		config.maxVelo = 3 + EnchantmentHelper.getLevel(OceanMaze.TRIDENT_BOW, stack);
		config.power = 1.5;
		config.punch = 3;
		...
		if (config.ammo.isEmpty())
			config.ammo = new ItemStack(Items.TRIDENT);
		int i = this.getMaxUseTime(stack) - remainingUseTicks;
		config.pullProgress = getPullProgress(i);
		if (config.pullProgress < 0.1)
			return;
		config.omitConsume = omitArrow && config.ammo.getItem() == Items.TRIDENT;
		if (!world.isClient) {
			config.ppe = new TridentEntity(config.world, config.player, config.ammo);
			config.ppe.setDamage(8f);
