...
public class Estimator {
	...
	private static class SucEstiRes implements EstiResult {
		...
		@Override
		public Vec3d getVec() {
			Vec3d fin = mov.dp.add(mov.ev.multiply(t)).multiply(1, 0, 1);
			double l = fin.length();
			double c = Math.cos(a);
			return new Vec3d(mov.v * c * fin.x / l, mov.v * Math.sin(a), mov.v * c * fin.z / l);
		}
	...
	private final Vec3d dp, ev;

	public Estimator(double G, double K, Vec3d pos, double V, int maxt, Vec3d ep, Vec3d eV) {
		gk = G / K;
		g = G;
		k = K;
		vk = V / K;
